...
(function ($) {
  plone.jscalendar = {
    ...
    show: function (input_id, yearStart, yearEnd) {
      ...
      if (!cal) {
        cal = plone.jscalendar._cal = new Calendar(
          ...
          1,
          null,
          plone.jscalendar.handle_select,
      ...
      if (fields.year.val() > 0) {
        cal.date.setFullYear(fields.year.val());
      }
      if (fields.month.val() > 0) {
        cal.date.setMonth(fields.month.val() - 1);
      }
      if (fields.day.val() > 0) {
        cal.date.setDate(fields.day.val());
      }
      cal.refresh();
    ...
    handle_select: function (cal, date) {
      ...
      if (
        $.nodeName(fields.year.get(0), "select") &&
        ...
        for (i = options.length; i > 0; i = i - 1) {
          if (options[i].value > yearValue) {
            options[i + 1] = new Option(options[i].value, options[i].text);
          } else {
    ...
    update_hidden: function (e) {
      ...
      if (e && e.target.selectedIndex === 0) {
        ...
        filter =
          $.inArray(type, ["hour", "minute", "ampm"]) > -1
            ? "select[id$=hour],select[id$=minute],select[id$=ampm]"
            : "select";
        $.each(f, function () {
          this.filter(filter).attr("selectedIndex", 0);
        });
        ...
        if (f.hour.length && f.minute.length) {
          val += " " + [f.hour.val(), f.minute.val(), "00"].join(":");
          if (f.ampm.length) {
            val += " " + f.ampm.val();
          }
        }
      }
      // #106898 add timezone as well in order to set value as given by
      // plone widget ex: 2023/05/18 22:55:00 GMT+2
      var original_date = f.field.val().split(" ");
      var timezone = original_date[original_date.length - 1];
      f.field.val(val + timezone);
    }
  };
})(jQuery);
...
jQuery(function ($) {
  ...
  $(".plone-jscalendar-popup").each(function () {
    var jqt = $(this),
      widget_id = this.id.replace("_popup", "").split("|").join("\\|"),
