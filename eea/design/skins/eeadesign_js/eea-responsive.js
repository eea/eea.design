...
jQuery(document).ready(function ($) {
  ...
  var throttle = window.underscore
    ? window.underscore.throttle
    : function (t, e) {
      var n;
      return function () {
        var i,
  ...
  var $eea_tabs_with_arrows = $(".eea-tabs-arrows"),
      ...
      no_transform_pattern = ".eea-tabs-soer";
  ...
  function make_tabs_into_accordions($tab, $tabs_panel) {
    if (!$tabs_panel.length || !$tab.not(no_transform_pattern).length) {
      return;
    }
    ...
    if (css) {
      ...
      css = tabbed_menu_found
        ? replaceAll(
          css,
          "tabbedmenu-panel",
          "eea-accordion-panels tabbed-accordion-menu"
        )
        : css;
    ...
    $tabs_tabs.each(function (idx, el) {
      ...
      var $result = $("<h2 />", {
        class:
          link && link.className.indexOf("current") !== -1
            ? "eea-accordion-title current no-anchor"
            : "eea-accordion-title no-anchor",
  ...
  function make_accordions_into_tabs($tab) {
    var $tabs_accordion = $tab.parent().find(".eea-tabs-transformed");
    if (!$tabs_accordion.length || !$tab.not(no_transform_pattern).length) {
      return;
    }
    $tabs_accordion.each(function (idx, item) {
      ...
      $tabs_accordions.each(function (idx, panel) {
        ...
        var $tabs = $parent.hasClass("eea-tabs-panels-soer")
          ? $(".eea-tabs-soer")
          : $panel.parent().prev();
      ...
      _.debounce(function () {
        ...
        if ($buttonnavbar.css("display") !== "none") {
          ...
          if (eea_tabs_with_arrows_found) {
            make_tabs_into_accordions(
              $eea_tabs_with_arrows,
              $(".eea-tabs-panels-arrows")
            );
          }
          if (soer_tabs_found) {
  ...
  if (!$navbar_header.children("#portal-logo-link-header").length) {
    $portal_logo_link
      .clone()
      .attr("id", "portal-logo-link-header")
      .prependTo($navbar_header);
  }

  // make accordions out of the left and right areas of faceted navigation pages
  var $faceted_left_column = $("#faceted-left-column").addClass(
    "eea-accordion-panels collapsed-by-default non-exclusive"
  );
  var $faceted_right_column = $("#faceted-right-column").addClass(
    "eea-accordion-panels collapsed-by-default non-exclusive"
  );
  $faceted_left_column
    .find(".faceted-widget")
    .addClass("widget-fieldset")
    .appendTo($faceted_right_column);
  $faceted_right_column
    .find(".faceted-widget")
    .addClass("widget-fieldset")
  ...
  $(
    "<div class='faceted-textwidget-place-current'><a href='#' class='eea-faceted-filter eea-section-trigger eea-section-trigger-right'>Filter »</a></div>"
  ).appendTo($faceted_text_widget.find("form"));
  $(".eea-section-trigger").click(function (e) {
    e.preventDefault();
    var section = e.target.className.indexOf("trigger-right")
      ? ".eea-right-section"
      : ".eea-left-section";
    $(section).prev().click();
  });
  ...
  function swipeHandler(ev) {
    ...
    var $faceted_slider = $(".eea-right-section-slider"),
      faceted_slider_is_active = $faceted_slider.hasClass(
  ...
  var make_siteaction_panel = function (
    $content,
    $parent,
    panel_id,
    use_only_children
  ...
  make_siteaction_panel(
    $("#portal-personaltools"),
    $parent,
    "tip-siteaction-user-menu",
    true
  );
  ...
  if ($right_section_container.length) {
    (function insert_section() {
      $right_section_container.each(function (idx, el) {
        ...
        var $el = $(el),
          $right_section_slider = $el.prev();
        if (!$right_section_slider.hasClass("eea-right-section-slider")) {
          $right_section_slider = $(
            '<div class="eea-section eea-right-section-slider eea-scrolling-toggle-visibility"><span class="eea-icon eea-icon-5x eea-icon-caret-left eea-icon-anim-horizontal animated"></span></div>'
          );
        ...
        $right_section_slider.click(function () {
          var $this = $(this);
          $this
            .toggleClass("eea-right-section-slider-active")
            .next()
            .toggleClass("eea-right-section-active eea-scrolling-keep-visible");
  ...
  function navScroll(ev) {
    ...
    if (st > lastScrollTop) {
      ...
      $items.each(function (idx, el) {
        if (
          el.className.indexOf("is-eea-hidden") === -1 &&
